...
def update_instance(instance_id: str) -> JobResult:
    """Reconcile pending member changes for one Coder instance."""

    return _update_instance(UUID(instance_id), worker_database.get_worker_session_maker())
...
def _update_instance(
    ...
    claim = _claim_update(instance_id, session_factory)
    if claim is None:
        return {"status": "noop"}

    # Reconcile outside the database transaction to avoid holding row locks over HTTP.
    try:
        reconcile_operation = reconcile or argocd.reconcile_instance_application
        argocd_application_name = reconcile_operation(
            instance_id,
            claim.attached_name,
