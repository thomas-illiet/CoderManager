...
def list_template_workspaces(
    ...
    *,
    ...
    """Return every remote workspace attached to one concrete template ID."""

    with CoderClient(instance_url) as client:
        client.authenticate_prepared_admin(password)
        return _template_workspaces(client, template_id, heartbeat)


def delete_template_workspaces(  # noqa: PLR0913
    instance_url: str,
    password: SecretStr,
    template_id: UUID,
    *,
    ...
    """Delete every remote workspace attached to one template and prove convergence."""
    ...
    try:
        while True:
            _require_before_deadline(deadline, "Coder template workspace deletion timed out")
            workspaces = _template_workspaces(client, template_id, heartbeat)
            if not workspaces:
                return tuple(sorted(deleted_ids))
            ...
            for workspace in workspaces:
                ...
                build = client.create_workspace_delete_build(workspace.id)
                if build.transition != "delete":
                    msg = "Coder template workspace deletion returned an unexpected transition"
                    raise CoderRequestError(msg)
                if build.status in TERMINAL_WORKSPACE_DELETE_FAILURES:
                    msg = "Coder template workspace deletion failed"
                    raise CoderRequestError(msg)
                if build.status != "deleted":
                    delete_builds.add(build.id)
...
def _template_workspaces(
    client: CoderClient,
    template_id: UUID,
    heartbeat: Callable[[], None] | None,
) -> tuple[CoderWorkspace, ...]:
    """Load a complete snapshot and retain only one template's workspaces."""

    workspaces = _all_workspaces(client, heartbeat)
    if any(workspace.template_id is None for workspace in workspaces):
        msg = "Coder workspace page did not expose template identities"
        raise CoderRequestError(msg)
    return tuple(workspace for workspace in workspaces if workspace.template_id == template_id)
